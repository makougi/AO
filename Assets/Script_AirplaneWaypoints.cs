...
public class Script_AirplaneWaypoints : MonoBehaviour {
	...
	private Vector3 UpdateWaypointImageUIPosition (GameObject waypointImageGO) {
		Vector3 conversionToScreenPoint = Camera.main.WorldToScreenPoint (waypointImageGO.GetComponent<Script_WaypointImage> ().GetWorldPosition ());
	...
	private void CreateWaypointGameObjects (GameObject dIPanel) {
		...
		foreach (Script_Waypoint wp in waypointsList) {
			...
			wpImage.GetComponent<Script_WaypointImage> ().SetWorldPosition (wp.GetWpVector3Position ());
			wpText.transform.localPosition = new Vector3 (0, -42, 0);
			wpText.GetComponent<Text> ().text = ""
				+ GetComponent<Script_AirplaneMain> ().GetId () + "\n"
